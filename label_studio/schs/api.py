...
@method_decorator(name='get', decorator=swagger_auto_schema(
        tags=["Invites"],
        operation_summary='Get schs invite link',
        operation_description='Get a link to use to invite a new member to an schs in Label Studio Enterprise.',
        responses={200: SchInviteSerializer()}
    ))
class SchInviteAPI(APIView):
    parser_classes = (JSONParser,)
    permission_required = all_permissions.schs_change

    def get(self, request, *args, **kwargs):
        org = get_object_with_check_and_log(self.request, Sch, pk=request.user.active_sch_id)
        self.check_object_permissions(self.request, org)
        ...
        serializer = SchInviteSerializer(data={'invite_url': invite_url, 'token': org.token})
        serializer.is_valid()
...
@method_decorator(name='post', decorator=swagger_auto_schema(
        tags=["Invites"],
        operation_summary='Reset schs token',
        operation_description='Reset the token used in the invitation link to invite someone to an schs.',
        responses={200: SchInviteSerializer()}
    ))
class SchResetTokenAPI(APIView):
    permission_required = all_permissions.schs_invite
    parser_classes = (JSONParser,)

    def post(self, request, *args, **kwargs):
        org = request.user.active_sch
        org.reset_token()
        logger.debug(f'New token for schs {org.pk} is {org.token}')
        invite_url = '{}?token={}'.format(reverse('user-signup'), org.token)
        serializer = SchInviteSerializer(data={'invite_url': invite_url, 'token': org.token})
        serializer.is_valid()
